...
Phase 1 compared four small, controlled training candidates against the unchanged Milestone 4 baseline on the established train/validation split only. The detailed diagnosis, exact configurations, validation metrics, per-class findings, and checkpoint selection are in `data/experiments/improvement_phase1/comparison.md`. Conservative clip-consistent augmentation was the provisional validation winner (70.37% accuracy, 0.701 macro F1, 0.7616 loss at epoch 30), but validation remained volatile and Toeloop F1 worsened. AdamW at 1e-4 weight decay produced the baseline trajectory; a validation-loss scheduler lowered loss but not selected-checkpoint macro F1 as much as augmentation; the combined scheduler never activated and reproduced augmentation exactly. No candidate was evaluated on test during selection. The subsequent one-time final evaluation is recorded below. No Phase 1 checkpoint has replaced the fixed production model.

### One-Time Final Held-Out Evaluation

After validation selection, the augmentation-only checkpoint received one final held-out evaluation using the unchanged Milestone 5 protocol; no other Phase 1 checkpoint was tested. Video accuracy was 71.99% and macro F1 0.719, versus 65.97% and 0.652 for the original model. Twelve-view group accuracy was 83.33%, versus 72.22% originally. The full comparison, per-class metrics, confusion matrix, camera/skater slices, and limitations are in `data/evaluation/phase1_augmentation_final/`. Both sets of results remain preserved. The production web checkpoint remains the original epoch-19 model until an explicit deployment decision.
